    ...
    public static function get($team): array
    {
        return [
            Forms\Components\ViewField::make('teamOwner')
                ->label(trans('account::messages.teams.edit.owner'))
                ->hiddenLabel()
                ->view('account::forms.components.team-owner', ['team' => $team]),
            SpatieMediaLibraryFileUpload::make('avatar')
                ->avatar()
                ->label(trans('account::messages.teams.edit.avatar'))
                ->disabled(fn() => auth('accounts')->user()->id !== $team->account_id)
                ->collection('avatar'),
            TextInput::make('name')
                ->label(trans('account::messages.teams.edit.name'))
                ->disabled(fn() => auth('accounts')->user()->id !== $team->account_id)
                ->required(),
        ];
    }
