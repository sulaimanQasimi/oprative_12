...
import {
    Package, Search, RefreshCw, Eye, Calendar, DollarSign,
    Filter, Receipt, CircleDollarSign, PackageX
} from 'lucide-react';

export default function StockOutcomesIndex({ auth, stockOutcomes = { data: [], links: [], total: 0 }, filters = {}, products = [], statistics = {} }) {
                                    ...
                                    <form onSubmit={handleSubmit} className="grid gap-4 md:grid-cols-3">
                                        ...
                                        <div className="flex space-x-4">
                                            <button
                                                type="submit"
                                                disabled={processing}
                                                className="px-4 py-2 bg-gradient-to-r from-red-500 to-pink-600 hover:from-red-600 hover:to-pink-700 text-white text-sm font-medium rounded-md shadow-md hover:shadow-lg"
                                            >
                                            ...
                                            <button
                                                type="button"
                                                onClick={handleReset}
                                                className="px-4 py-2 bg-gradient-to-r from-gray-500 to-gray-600 hover:from-gray-600 hover:to-gray-700 text-white text-sm font-medium rounded-md shadow"
                                            >
                                                <span className="flex items-center justify-center">
                                        ...
                                        <table className="min-w-full divide-y divide-gray-200 dark:divide-slate-700">
                                            ...
                                            <tbody className="bg-white dark:bg-slate-900 divide-y divide-gray-100 dark:divide-slate-800">
                                                {stockOutcomes?.data?.length === 0 ? (
                                                    ...
                                                    stockOutcomes.data.map((outcome) => (
                                                        <tr key={outcome.id} className="hover:bg-red-50/30 dark:hover:bg-red-900/10 transition-all duration-300">
                                                            <td className="px-8 py-5 whitespace-nowrap">
                                                                <div className="text-sm text-gray-900 dark:text-white">{outcome.reference_number}</div>
                                                            </td>
                                                            <td className="px-6 py-5 whitespace-nowrap">
                                                                <div className="text-sm text-gray-900 dark:text-white">{outcome.product?.name || 'N/A'}</div>
                                                            </td>
                                                            <td className="px-6 py-5 whitespace-nowrap text-right">
                                                                <div className="text-sm text-gray-900 dark:text-white">{outcome.quantity}</div>
                                                            </td>
                                                            <td className="px-6 py-5 whitespace-nowrap text-right">
