...
export default function Outcome({ account, outcomes, filters, auth }) {
    ...
    const handleSearch = () => {
        setLoading(true);
        router.get(route('admin.accounts.outcomes', account.id), {
            search,
    ...
    const handleDelete = (id) => {
        if (confirm(t('Are you sure you want to delete this outcome record? This action cannot be undone.'))) {
            setLoading(true);
            router.delete(route('admin.outcomes.destroy', id), {
                onFinish: () => setLoading(false)
            });
        }
    };
    ...
    const totalApprovedOutcomes = outcomes.data?.filter(outcome => outcome.status === 'approved').length || 0;
    const totalPendingOutcomes = outcomes.data?.filter(outcome => outcome.status === 'pending').length || 0;
    const totalOutcomeAmount = outcomes.data?.reduce((sum, outcome) => sum + (outcome.status === 'approved' ? parseFloat(outcome.amount) : 0), 0) || 0;

    return (
        <>
            <Head title={t(":name - Outcome Management", { name: account.name })}>
                <style>{`
                    .glass-effect {
                        ...
                        <div className="flex justify-between items-start">
                            <div className="space-y-1">
                                ...
                                <p className="text-gray-600 dark:text-gray-400">
                                    {t("Manage outcome transactions for account #:number", { number: account.account_number })}
                                </p>
                            </div>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                                        ...
                                        <CardContent className="p-6">
                                            <div className="flex items-center justify-between">
                                                <div>
                                                    ...
                                                    <p className="text-3xl font-bold text-green-700">{totalApprovedOutcomes}</p>
                                                </div>
                                        ...
                                        <CardContent className="p-6">
                                            <div className="flex items-center justify-between">
                                                <div>
                                                    ...
                                                    <p className="text-3xl font-bold text-orange-700">{totalPendingOutcomes}</p>
                                                </div>
                                    ...
                                    <Card className="bg-white/80 dark:bg-slate-800/80 backdrop-blur-xl border border-white/20 dark:border-slate-700/50">
                                        <CardContent className="p-6">
                                            <div className="flex items-center justify-between">
                                                <div>
                                                    <p className="text-sm font-medium text-red-600">{t("Total Amount")}</p>
                                        ...
                                        <CardContent className="p-6">
                                            <div className="flex items-center justify-between">
                                                <div>
                                                    ...
                                                    <p className="text-3xl font-bold text-purple-700">{outcomes.total || 0}</p>
                                                </div>
                                    ...
                                    <CardContent className="space-y-4">
                                        <div className="flex gap-4">
                                            <div className="flex-1">
                                                <Input
                                                    placeholder={t("Search by description, amount, or reference...")}
                                                    value={search}
                                ...
                                <Card className="bg-white/80 dark:bg-slate-800/80 backdrop-blur-xl border border-white/20 dark:border-slate-700/50">
                                    <CardHeader>
                                        <CardTitle className="flex items-center gap-2">
                                            ...
                                            {t("Outcome Records")}
                                            <Badge variant="secondary">{outcomes.total || 0}</Badge>
                                        </CardTitle>
                                    </CardHeader>
                                    <CardContent>
                                        {outcomes.data && outcomes.data.length > 0 ? (
                                                            ...
                                                            <TableRow key={outcome.id}>
                                                                ...
                                                                <TableCell>
                                                                    <div>
                                                                        ...
                                                                        {outcome.notes && (
                                                                            <p className="text-sm text-gray-500">{outcome.notes}</p>
                                                                        )}
                                                                    </div>
                                                                </TableCell>
                                            ...
                                            <div className="text-center py-12">
                                                ...
                                                <p className="text-gray-500 mb-4">
                                                    {t("Start by adding your first outcome transaction for this account.")}
                                                </p>
                                                <Button className="gap-2">
                                                    <Plus className="w-4 h-4" />
                                                    {t("Add First Outcome")}
