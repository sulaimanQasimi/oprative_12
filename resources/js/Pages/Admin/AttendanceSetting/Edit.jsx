...
export default function Edit({ auth, attendanceSetting, permissions = {} }) {
    const { t } = useLaravelReactI18n();
    const [loading, setLoading] = useState(true);
    const [isAnimated, setIsAnimated] = useState(false);

    const { data, setData, put, processing, errors, reset } = useForm({
        enter_time: attendanceSetting.enter_time || "",
        exit_time: attendanceSetting.exit_time || "",
        date: attendanceSetting.date || "",
    });
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        put(route("admin.attendance-settings.update", attendanceSetting.id));
    };

    return (
        <>
            <Head title={`${t("Edit Attendance Setting")}`}>
                <style>{`
                    .glass-effect {
                                            ...
                                            <div className="flex items-center justify-end space-x-4 pt-6">
                                                ...
                                                {permissions.update_attendance_setting && (
                                                    ...
                                                    >
                                                        ...
                                                        {processing ? t("Updating...") : t("Update Setting")}
                                                    </Button>
                                                )}
                                            </div>
                                        </form>
