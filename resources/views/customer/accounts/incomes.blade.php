<x-customer-layout>
    <div class="max-w-7xl mx-auto py-6 sm:px-6 lg:px-8">
        ...
        <div class="md:flex md:items-center md:justify-between mb-8 pb-4 border-b border-gray-200">
            ...
            <div class="mt-4 flex md:mt-0 md:ml-4 space-x-6 space-x-reverse">
                ...
                <button onclick="document.getElementById('createIncomeModal').classList.remove('hidden')"
        ...
        <div class="mb-8">
            <div class="border-b border-gray-200">
                <nav class="-mb-px flex space-x-8 space-x-reverse" aria-label="Tabs">
                    <a href="{{ route('customer.accounts.show', ['account' => $account->id, 'tab' => 'incomes']) }}"
                        class="group relative whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm transition-all duration-300 {{ request()->get('tab') !== 'outcomes' ? 'border-indigo-500 text-indigo-600' : 'border-transparent text-gray-500 hover:text-gray-700' }}">
                        <span class="relative z-10">@lang('Incomes')</span>
                        <div class="absolute inset-0 bg-indigo-50 transform scale-x-0 group-hover:scale-x-100 transition-transform duration-300 origin-right {{ request()->get('tab') !== 'outcomes' ? 'scale-x-100' : '' }}"></div>
                    </a>
                    <a href="{{ route('customer.accounts.show', ['account' => $account->id, 'tab' => 'outcomes']) }}"
                        class="group relative whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm transition-all duration-300 {{ request()->get('tab') === 'outcomes' ? 'border-indigo-500 text-indigo-600' : 'border-transparent text-gray-500 hover:text-gray-700' }}">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                        <div class="flex items-center justify-end space-x-4 space-x-reverse">
                                            @if($income->status === 'pending')
                                                <form action="{{ route('customer.accounts.incomes.approve', ['account' => $account->id, 'income' => $income->id]) }}" method="POST" class="inline">
                                                    @csrf
                                                    <button type="submit"
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                        <div class="flex items-center justify-end space-x-4 space-x-reverse">
                                            @if($outcome->status === 'pending')
                                                <form action="{{ route('customer.accounts.outcomes.approve', ['account' => $account->id, 'outcome' => $outcome->id]) }}" method="POST" class="inline">
                                                    @csrf
                                                    <button type="submit"
                                                        class="inline-flex items-center px-3 py-1.5 border border-transparent text-sm font-medium rounded-md text-white bg-green-600 hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500 transition-all duration-200 transform hover:scale-105">
                                                        <svg class="w-4 h-4 ml-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <!-- Create Income Modal -->
    <div id="createIncomeModal" class="fixed z-10 inset-0 overflow-y-auto hidden">
        <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
            ...
            <div class="inline-block align-bottom bg-white rounded-2xl text-right overflow-hidden shadow-2xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
                <form action="{{ route('customer.accounts.incomes.store', $account->id) }}" method="POST">
                    @csrf
                    <!-- Header -->
                    <div class="bg-gradient-to-r from-indigo-600 via-purple-600 to-blue-600 px-6 py-4">
                        <div class="flex items-center justify-between">
                            ...
                            <button type="button" onclick="document.getElementById('createIncomeModal').classList.add('hidden')" class="text-white hover:text-indigo-100 focus:outline-none">
                    ...
                    <div class="bg-gray-50 px-6 py-4 flex justify-end space-x-3 space-x-reverse">
                        <button type="button" onclick="document.getElementById('createIncomeModal').classList.add('hidden')"
