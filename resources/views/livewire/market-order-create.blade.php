<div class="bg-white/80 backdrop-blur-lg rounded-2xl p-6 shadow-lg">
    ...
    @if($currentOrderId)
        ...
        <div class="mb-6 relative">
            <div
                class="flex items-center bg-white/90 rounded-xl shadow-sm overflow-hidden border border-gray-300 focus-within:border-blue-500 focus-within:ring-1 focus-within:ring-blue-500">
            ...
            @if($searchQuery && $showDropdown)
                <div
                    class="absolute z-50 w-full mt-1 bg-white rounded-xl shadow-lg border border-gray-200 max-h-96 overflow-y-auto">
                    @forelse($searchResults as $index => $product)
                        <div class="flex justify-between items-center w-full">
                            ...
                            <button wire:click="saveToOrder({{ $index }})"
                                class="ml-4 px-3 py-1 bg-green-500 text-white rounded-lg hover:bg-green-600 transition-colors duration-200">
                                Save to Order
                            </button>
                        </div>
                    @empty
